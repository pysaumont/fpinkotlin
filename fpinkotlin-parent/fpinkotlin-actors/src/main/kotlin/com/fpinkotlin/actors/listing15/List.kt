...
sealed class List<out A> {
    ...
    fun zipWithPosition(): List<Pair<A, Int>> =
        zipWith(this, range(0, this.length)) { a ->
            { i: Int ->
                Pair(a, i)
            }
        }
...
//    }
    ...
    fun getAt(index: Int): Result<A> {
        ...
        return Pair<Result<A>, Int>(
            Result.failure("Index out of bound"), index).let { identity ->
    ...
    fun filter(p: (A) -> Boolean): List<A> = flatMap { a -> if (p(a)) List(
        a) else Nil
    }

    abstract fun take(n: Int): List<A>

    internal object Nil: List<Nothing>() {

        override fun take(n: Int): List<Nothing> = this
    ...
    internal class Cons<out A>(internal val head: A,
        ...
        override fun take(n: Int): List<A> = when {
            this.isEmpty() -> this
                ...
                n > 1 -> Cons(head, tail.take(n - 1))
                else  -> Cons(head, Nil)
            }
        }

        override fun tailSafe(): Result<List<A>> = Result(
            tail)
    ...
    companion object {
        ...
        tailrec fun <A, B> coFoldRight(acc: B, list: List<A>, identity: B, f: (A) -> (B) -> B): B =
                when (list) {
                    Nil     -> acc
                    is Cons -> coFoldRight(
                        f(list.head)(acc), list.tail, identity, f)
                }


        operator fun <A> invoke(vararg az: A): List<A> =
                az.foldRight(Nil) { a: A, list: List<A> ->
                    Cons(a, list)
                }
