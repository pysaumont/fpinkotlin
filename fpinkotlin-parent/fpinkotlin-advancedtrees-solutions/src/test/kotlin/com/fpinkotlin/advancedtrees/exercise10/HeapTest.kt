...
class HeapTest: StringSpec() {
    ...
    init {
        ...
        "test Plus Ordered Ascending 7 Comparator" {
            ...
            heap.size shouldBe list.length()
        }
...
private fun <A> rightSpine(heap: Heap<A>): Int {
    return 1 + rightSpine(heap.right)
}

private fun <A> rightSpine(heap: Result<Heap<A>>): Int {
    return heap.map({ t -> if (t.isEmpty) -1 else 1 + rightSpine(t.right) }).getOrElse(-1)
}

private fun <A> isValueOrdered(heap: Heap<A>): Boolean =
    isValueOrderedHelper(heap) && heap.tail().map { isValueOrderedHelper(it) }.getOrElse(true)

private fun <A> isValueOrderedHelper(heap: Heap<A>): Boolean =
...
private fun <A> isValueOrdered(heap: Heap<A>, comparator: Comparator<A>): Boolean =
    isValueOrderedHelper(heap, comparator) && heap.tail().map { isValueOrderedHelper(it, comparator) }.getOrElse(true)

private fun <A> isValueOrderedHelper(heap: Heap<A>, comparator: Comparator<A>): Boolean =
