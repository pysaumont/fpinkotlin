...
import benchmark.log2nlz
import benchmarkJ.Tree.Companion.empty
...
abstract class Tree<A: Comparable<A>> {
    ...
    internal abstract fun ins(a: A): Tree<A>
    internal abstract operator fun rem(a: A): Tree<A>
    internal abstract fun mrg(a: Tree<A>): Tree<A>

    abstract fun value(): A
    abstract fun insert(a: A): Tree<A>
    abstract fun member(a: A): Boolean
    abstract fun size(): Int
    abstract fun height(): Int
    abstract fun max(): Result<A>
    abstract fun min(): Result<A>
    abstract fun remove(a: A): Tree<A>
    abstract fun merge(a: Tree<A>): Tree<A>
    abstract fun merge(a: A, right: Tree<A>): Tree<A>
     ...
     */
    protected abstract fun removeMerge(ta: Tree<A>): Tree<A>
...
//    abstract fun <B: Comparable<B>> map(f: (A) -> B): Tree<B>
    ...
    private class Empty<A: Comparable<A>>: Tree<A>() {
        ...
        override fun insert(value: A): Tree<A> {
            return T(empty(), value, empty())
        }

        override fun ins(a: A): Tree<A> {
            return insert(a)
        }

        override fun member(a: A): Boolean {
            return false
        }
        ...
        override fun merge(a: A, right: Tree<A>): Tree<A> {
            return right.min().map { min ->
...
//        }
    ...
    private class T<A: Comparable<A>> constructor(private val left: Tree<A>,
                                                          private val value: A,
                                                          private val right: Tree<A>): Tree<A>() {
        ...
        override fun insert(a: A): Tree<A> {
            val t = ins(a)
        ...
        override fun merge(a: A, right: Tree<A>): Tree<A> {
            return if (right.isEmpty)
                max().map({ max ->
                              if (max.compareTo(a) > 0 // Default value can never be used, but is necessary
                              )
                                  insert(a)
                              else
                                  T(this, a, right)
